...
function AdminProduct() {
    ...
    return (
        ...
        <div className="container">
            ...
            <table className='table table-hover table-striped'>
                ...
                {products.map((item) => (
                    ...
                    <td>{item.stars || '---'}<span className="text-warning">★</span></td>
                    <td>
                        <span className={item.is_enabled ? 'text-success' : 'text-secondary'}>
                        {item.is_enabled ? '啟用' : '未啟用'}
                        </span>
                    </td>
                    <td>
                        <button type='button' className='btn btn-primary me-3 btn-sm' onClick={() => handleOpenModal("edit", item)}>編輯</button>
                        <button type='button' className='btn btn-danger btn-sm' onClick={() => handleOpenModal("del", item)}>刪除</button>
                        </td>
                    </tr>
                ))}
                </tbody>
        ...
        <Modal modalType={modalType} templateData={templateData} 
        isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} 
        getProducts={getProducts} currentPage={pageInfo.current_page || 1} />
      </>
    )
};
